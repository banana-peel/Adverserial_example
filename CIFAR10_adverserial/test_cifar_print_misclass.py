...
xp_test = tf.expand_dims(data[9],0)
...
adv_op_acr = tf.argmax(tf.squeeze(adv_op))
orig_op_acr = tf.argmax(tf.squeeze(orig_op))
label_acr = tf.argmax(tf.squeeze(data_label))
