...
adv_lab = tf.argmax(adv_op,1)   #check this out if it has to be [1] or [0]
orig_lab = tf.argmax(orig_op,1)
...
#print(sess.run(adv_op, feed_dict={data:x_test,data_label:y_test, K.learning_phase(): 0}))
#print(sess.run(orig_op, feed_dict={data:x_test,data_label:y_test, K.learning_phase(): 0}))
#print(y_test[0])
#score = loaded_model.evaluate(test_image, data_label_slc, verbose=0)
print("%.2f%%" % (score*100))
#score = loaded_model.evaluate(xp_test, data_label_slc, verbose=0)
print("%.2f%%" % (score*100))
